...
void TankDrawable::update(const std::shared_ptr<IObject> &object) {
  ...
  uint8_t color = state >> 3 & 3;
  bool wheelState = state >> 2 & 1;
  uint8_t rotation = state & 3;

  if (object->getType() == ObjectType::PlayerTank) {
    _sprites[0].setTextureRect(sf::IntRect(color * 128 + rotation * 32 + wheelState * 16, tankType * 16, 16, 16));
  }
  else {
    _sprites[0].setTextureRect(sf::IntRect(color * 128 + rotation * 32 + wheelState * 16, 64 + tankType * 16, 16, 16));
  }
